...
function love.load(argv)
  ...
  if love.system.getOS() == 'Android' then
    ...
    love.window.setMode(
      __pico_resolution[1] * scale + xpadding * scale * 2,
      __pico_resolution[2] * scale + ypadding * scale * 2
    )
  end
  ...
  if filename:sub(#filename - 3, #filename) == '.png' then
      ...
      for x = 0, 159 do
        ...
        elseif inbyte < 0x4300 then
          ...
          if step < 64 and inbyte % 2 == 1 then
            ...
            __pico_sfx[_sfx][(step - 1) / 2] = {
              bit.band(note, 0x3f),
              bit.rshift(bit.band(note, 0x01c0),  6),
              bit.rshift(bit.band(note, 0x0e00),  9),
    ...
    while next_line do
      ...
      for i = 1, #line do
        ...
        v = tonumber(v, 16)
        __pico_spritesheet_data:setPixel(col, row,
          v * 16, v * 16, v * 16, 255)
...
function mget(x, y)
  ...
  if y > 63 or x > 127 or x < 0 or y < 0 then
    return 0
  end
